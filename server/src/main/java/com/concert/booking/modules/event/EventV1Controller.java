...
public class EventV1Controller {
     ...
     */
    @Operation(summary = "Tạo hạng vé (Ticket Classes)", 
     ...
     */
    @Operation(summary = "Cập nhật giá vé", 
            description = "Cập nhật giá vé cho hạng vé.")
    @BadRequestApiResponse
    @PatchMapping("/ticket-classes/{ticketClassId}/price")
    public DataApiResponse<TicketClass> updateTicketClassPrice(
            @PathVariable UUID ticketClassId, 
            @RequestBody @Valid TicketClassUpdateDTO dto) {
        UUID currentUserId = AuthUtils.getCurrentUserId();
        TicketClass ticketClass = eventService.updateTicketClassPrice(ticketClassId, dto, currentUserId);
        return DataApiResponse.success(ticketClass, "Cập nhật giá vé thành công");
    }

    /**
     * Lưu sơ đồ ghế
     * Layout Safety: Không được xóa/thay đổi ticket class của ghế SOLD
     */
    @Operation(summary = "Lưu sơ đồ ghế", 
     ...
     * Maintenance: Ghế hỏng vật lý, không thể chọn mua nhưng giữ vị trí
     */
    @Operation(summary = "Cập nhật trạng thái ghế", 
    ...
    public DataApiResponse<Void> updateSeatStatus(
            ...
            @RequestBody @Valid SeatStatusUpdateDTO dto) {
        UUID currentUserId = AuthUtils.getCurrentUserId();
     ...
     */
    @Operation(summary = "Cập nhật trạng thái sự kiện", 
