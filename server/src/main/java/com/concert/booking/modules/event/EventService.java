...
public interface EventService {
  ...
  /**
   * [US-A03] Định giá vé - Tạo tối thiểu 3 hạng vé - Ràng buộc: Phải ít nhất 3 hạng vé - Price
   * Locking: Không được chỉnh sửa giá nếu đã bán ít nhất 1 vé
   */
  List<TicketClass> defineTicketClasses(
  ...
  /**
   * [US-A03] Lưu sơ đồ ghế - Layout Safety: Không được xóa hoặc thay đổi ticket class của ghế SOLD
   * - Hỗ trợ MAINTENANCE status cho ghế bị hỏng vật lý
   */
  void saveSeatLayout(UUID eventId, SeatLayoutDTO dto, UUID createdBy);

  /**
   * [US-A03] Cập nhật trạng thái ghế (đặc biệt là MAINTENANCE) - Maintenance: Ghế chuyển sang màu
   * xám, không thể chọn mua nhưng vẫn giữ vị trí
   */
  void updateSeatStatus(UUID seatId, String status, UUID updatedBy);

  /**
   * [US-A03] Cập nhật trạng thái sự kiện thủ công - Có thể cập nhật khi sự kiện chưa ENDED hoặc
   * CANCELED
   */
