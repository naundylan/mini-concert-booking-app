...
public class EventServiceImpl implements EventService {
  ...
  /**
   * [US-A03] Tạo sự kiện mới ở trạng thái DRAFT - Ràng buộc: teasingTime <= openTime <= startTime
   * <= endTime
   */
  @Override
  @Transactional
  public Event createEvent(EventCreateDTO dto, UUID createdBy) {
    ...
    if (dto.getOpenTime().after(dto.getStartTime())) {
      throw new AppException(
          HttpStatus.BAD_REQUEST, "Thời gian mở bán không được trễ hơn thời gian diễn ra");
    }

    if (dto.getTeasingTime().after(dto.getOpenTime())) {
      throw new AppException(
          HttpStatus.BAD_REQUEST, "Thời gian Teasing không được trễ hơn thời gian mở bán");
    }

    if (dto.getStartTime().after(dto.getEndTime())) {
      throw new AppException(
          HttpStatus.BAD_REQUEST, "Thời gian kết thúc không được sớm hơn thời gian bắt đầu");
    }
  ...
  /**
   * [US-A03] Định giá vé - Tạo tối thiểu 3 hạng vé AC: Phải tạo tối thiểu 3 hạng vé (VIP,
   * Standard...) AC: Price Locking - Không được chỉnh sửa giá nếu đã bán ít nhất 1 vé
   */
  @Override
  @Transactional
  public List<TicketClass> defineTicketClasses(
      UUID eventId, List<TicketClassCreateDTO> dtos, UUID createdBy) {
    Event event =
        eventRepository
            .findById(eventId)
    ...
    if (event.getStatus() == EventStatus.ONSALE || event.getStatus() == EventStatus.ENDED) {
      ...
      if (hasSoldSeats) {
        throw new AppException(
            HttpStatus.BAD_REQUEST, "Sự kiện đã bán vé, không thể thiết lập lại giá hoặc hạng vé.");
      }
    }
  ...
  /**
   * [US-A03] Cập nhật giá vé cho hạng vé cụ thể AC: Price Locking - Không được cập nhật nếu có vé
   * SOLD cho ticket class này
   */
  @Override
  @Transactional
  public TicketClass updateTicketClassPrice(
      UUID ticketClassId, TicketClassUpdateDTO dto, UUID updatedBy) {
  ...
  /**
   * [US-A03] Lưu sơ đồ ghế AC: Layout Safety - Không được xóa hoặc đổi Hạng vé của ghế SOLD AC: Hỗ
   * trợ MAINTENANCE status cho ghế bị hỏng vật lý (không thể chọn mua nhưng giữ vị trí)
   */
  @Override
  @Transactional
            ...
            .map(
                s ->
                    Seat.builder()
                        ...
                        .label(
                            (char) ('A' + s.getGridRow()) + String.valueOf(s.getGridColumn() + 1))
  ...
  /**
   * [US-A03] Cập nhật trạng thái ghế (đặc biệt là MAINTENANCE) AC: Maintenance - Ghế chuyển sang
   * màu xám, không thể chọn mua nhưng vẫn giữ nguyên vị trí
   */
  @Override
  @Transactional
  public void updateSeatStatus(UUID seatId, String status, UUID updatedBy) {
    ...
    if (seat.getStatus() == SeatStatus.SOLD) {
      throw new AppException(
          HttpStatus.BAD_REQUEST, "Không thể thay đổi trạng thái của ghế đã bán");
    }
    ...
    try {
      ...
      throw new AppException(HttpStatus.BAD_REQUEST, "Trạng thái ghế không hợp lệ");
    }
  }

  /**
   * [US-A03] Cập nhật trạng thái sự kiện thủ công Vòng đời: DRAFT -> TEASING -> ONSALE -> ENDED AC:
   * Không thể đổi trạng thái nếu sự kiện đã ENDED hoặc CANCELED
   */
  @Override
  @Transactional
  ...
  public Event updateEvent(UUID eventId, EventUpdateDTO dto, UUID updatedBy) {
    ...
    if (currentStatus == EventStatus.CANCELED || currentStatus == EventStatus.ENDED) {
      throw new AppException(
          HttpStatus.BAD_REQUEST, "Không thể chỉnh sửa sự kiện đã hủy hoặc đã kết thúc");
    }
    ...
    if (currentStatus == EventStatus.ONSALE) {
      if (dto.getStatus() != null && dto.getStatus() == EventStatus.CANCELED) {
        ...
        throw new AppException(HttpStatus.BAD_REQUEST, "Sự kiện đang ONSALE chỉ có thể bị hủy");
      }
      event.setUpdatedBy(updatedBy);
      return eventRepository.save(event);
    }
    ...
    if (currentStatus == EventStatus.TEASING) {
      if (dto.getStatus() != null && dto.getStatus() == EventStatus.CANCELED) {
        ...
        throw new AppException(HttpStatus.BAD_REQUEST, "Sự kiện đang TEASING chỉ có thể bị hủy");
      }
