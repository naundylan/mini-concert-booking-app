...
public interface TicketClassRepository extends JpaRepository<TicketClass, UUID> {
    ...
    /**
     * Kiểm tra xem ticket class có vé đã bán hay không (Price Locking check)
     * Nếu có 1 vé SOLD cho ticket class này => khóa giá
     */
     ...
     */
    @Query("SELECT DISTINCT s.ticketClassId FROM Seat s WHERE s.eventId = :eventId AND s.status = 'SOLD'")
